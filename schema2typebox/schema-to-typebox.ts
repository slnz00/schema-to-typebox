...
import {
  ...
  isUnknownSchema, isStringEnumSchema
...
export const collect = (schema: JSONSchema7Definition): Code => {
  ...
  } else if (isStringEnumSchema(schema)) {
    return parseStringEnum(schema);
...
const createImportStatements = () => {
  return [
    'import { Kind, SchemaOptions, Static, TSchema, TUnion, Type, TypeRegistry, StringEnum } from "@feathersjs/typebox"',
...
export const parseStringEnum = (schema: EnumSchema) => {
  const schemaOptions = parseSchemaOptions(schema);
  const code = schema.enum.reduce<string>((acc, value) => {
    return acc + `${acc === "" ? "" : ","} '${value}'`;
  }, "");
  return schemaOptions === undefined
    ? `StringEnum([${code}])`
    : `StringEnum([${code}], ${schemaOptions})`;
};
